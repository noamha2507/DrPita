...
> **בנוסף**, כל מסך UI פותח בהערה `// [מסך UI · FRx] ...` וכל API Route פותח
> בהערה `// [API Route · FRx] ...` — כך ש-`grep -rn "FR1"` (או FR2/FR3) מחזיר
> את כל השרשרת מלמעלה למטה: מסך → API Route → Controller → הודעות ה-Sequence
> במודלים, לפי הסדר המדויק שבו מציגים תרחיש בהגנה.
